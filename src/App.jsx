...
import {
  ...
  Award, BarChart, Smartphone, FileText, ArrowUpRight
} from 'lucide-react';
...
// 🟢 之前缺失的变量：师傅任务数据 (Job Management Data)
const sifuSchedule = [
  { id: 101, type: "SOS", status: "active", time: "14:30", customer: "Mdm. Lee", category: "爆水管 / 漏水", address: "B-12-05, Pangsapuri Indah", price: "RM 120", date: "Today" },
  { id: 102, type: "Booking", status: "pending", time: "16:00", customer: "Jason Lim", category: "冷气清洗 (2台)", address: "No. 8, Jalan Kenari, Puchong", price: "RM 200", date: "Today" },
  { id: 103, type: "Booking", status: "completed", time: "10:00", customer: "Ali Ahmad", category: "更换插座", address: "Taman Equine, Shop Lot 2", price: "RM 80", date: "Today" },
  { id: 104, type: "Booking", status: "pending", time: "09:00", customer: "Sarah Tan", category: "安装吊扇", address: "Cyberjaya Domain 5", price: "RM 150", date: "Tomorrow" }
];
...
const initialUserRequests = [
  { id: 1, user: "Mdm. Lee", avatar: null, category: "冷气服务", title: "两台 Daikin 冷气不冷了", desc: "开了16度还是感觉像送风，而且出风口有点漏水，需要清洗和检查 gas。", photos: ["https://images.unsplash.com/photo-1545259741-2ea3ebf61fa3?auto=format&fit=crop&w=400&q=80"], dist: "1.2km", address: "Taman Equine, Seri Kembangan", time: "10分钟前", status: "open", comments: [{ id: 101, sifu: "王师傅", text: "老板，这个情况应该是要做 Chemical wash 了。普通洗 RM100，药水洗 RM150。", time: "5分钟前", priceOffer: "RM 150" }] },
  { id: 2, user: "Jason Lim", avatar: null, category: "房屋修缮", title: "厨房洗手盆下面漏水", desc: "应该是水管老化了，整个柜子都湿了。需要换新的水管。", photos: ["https://images.unsplash.com/photo-1585704032915-c3400ca199e7?auto=format&fit=crop&w=400&q=80"], dist: "3.5km", address: "Puchong Jaya", time: "2小时前", status: "open", comments: [] }
];
        ...
        return (
            <div className="flex flex-col h-full bg-brand-blue text-white animate-fade-in relative overflow-hidden md:flex-row">
                <div className="flex-1 flex flex-col items-center justify-center p-8 z-10 md:items-start md:pl-20">
                    ...
                    <p className="text-blue-100 text-center mb-12 md:text-left md:text-xl">马来西亚首选家庭服务平台。<br/>解决生活琐事，只需一键呼叫。</p>
                    <div className="hidden md:block text-blue-200 text-sm mt-8">💻 网页版 / 📱 手机 App 全平台支持</div>
                </div>
        ...
        <div className="flex flex-col h-full bg-white animate-slide-up items-center justify-center">
            <div className="w-full max-w-md p-6">
                ...
                <div className="space-y-6">
                    ...
                    <button onClick={handleLogin} className={`w-full py-4 rounded-xl font-bold text-white shadow-lg transition active:scale-95 ${mobile.length > 8 ? (role === 'user' ? 'bg-brand-blue' : 'bg-gray-900') : 'bg-gray-300 cursor-not-allowed'}`}>{role === 'user' ? '获取验证码' : '下一步: 验证身份'}</button>
                </div>
                {role === 'sifu' && (<div className="mt-8 p-4 bg-gray-50 rounded-xl"><h4 className="font-bold text-sm mb-2">师傅注册要求:</h4><ul className="text-xs text-gray-500 space-y-1 list-disc pl-4"><li>马来西亚公民 (IC) 或合法工作准证</li><li>拥有相关技能证书 (如适用)</li><li>无犯罪记录</li></ul></div>)}
            </div>
        </div>
    );
};
...
const SifuDashboard = ({ onLogout }) => {
	...
	const walletTransactions = [
	  { id: 1, type: "income", title: "订单收入 - #ORD-8823", date: "今天, 15:30", amount: "+ RM 120.00", status: "success" },
	  { id: 2, type: "income", title: "订单收入 - #ORD-8810", date: "今天, 11:00", amount: "+ RM 80.00", status: "success" },
	  { id: 3, type: "withdraw", title: "提现至 Maybank", date: "昨天", amount: "- RM 500.00", status: "processing" },
	  { id: 4, type: "fee", title: "平台服务费 (5%)", date: "昨天", amount: "- RM 10.00", status: "success" },
	  { id: 5, type: "income", title: "订单收入 - #ORD-8755", date: "10月2日", amount: "+ RM 250.00", status: "success" },
	];
            ...
            <div className="flex flex-col h-full bg-gray-900 text-white animate-slide-up">
                ...
                <div className="p-6 pt-12 pb-4">
                        ...
                        <div className="relative z-10">
                            ...
                            <div className="flex gap-3">
                                <button className="flex-1 bg-white text-brand-blue py-2.5 rounded-xl font-bold text-sm shadow-lg active:scale-95 transition flex items-center justify-center gap-2">
                                    <ArrowRight size={16} className="-rotate-45"/> 提现
                                </button>
                    ...
                    <div className="mt-4 bg-gray-800 p-4 rounded-2xl border border-gray-700 flex justify-between items-center shadow-md">
                        ...
                        <div className="text-right">
                            ...
                            <p className="text-[10px] text-gray-500">对比上周 RM 630</p>
                        </div>
                    </div>
                </div>
				...
				<div className="h-full overflow-auto">
					<div className="flex-1 bg-gray-800 rounded-t-3xl p-6 overflow-hidden flex flex-col">
						<h3 className="font-bold text-white mb-4">交易记录</h3>
							...
							{walletTransactions.map((t) => (
								<div key={t.id} className="flex items-center justify-between group">
									...
									<div className="text-right">
										...
										}`}>{t.status === 'success' ? '成功' : '处理中'}</div>
									</div>
								</div>
							))}
							<div className="pt-4 text-center">
								<button className="text-xs text-gray-500 hover:text-white transition">查看全部历史记录</button>
							</div>
						</div>
					</div>
                </div>
        ...
        <div className="fixed inset-0 z-[60] bg-white flex flex-col animate-slide-up md:absolute md:inset-0">
            ...
            <div className="flex-1 overflow-y-auto p-4 pb-24">
                ...
                <div className="border-t border-gray-100 pt-4">
                    ...
                    <div className="space-y-4">{selectedRequest.comments.map(c => (<div key={c.id} className="flex gap-3"><div className="w-8 h-8 bg-gray-200 rounded-full flex-shrink-0"></div><div className="bg-gray-50 p-3 rounded-xl rounded-tl-none flex-1"><div className="flex justify-between items-baseline mb-1"><span className="text-xs font-bold text-gray-800">{c.sifu}</span><span className="text-[10px] text-gray-400">{c.time}</span></div><p className="text-sm text-gray-600">{c.text}</p>{c.priceOffer && <div className="mt-2 text-brand-blue font-bold text-sm">报价: {c.priceOffer}</div>}</div></div>))}</div>
                </div>
            </div>
            <div className="p-4 bg-white border-t border-gray-100 pb-8"><div className="flex gap-2"><input type="text" value={replyText} onChange={e => setReplyText(e.target.value)} placeholder="询问详情或直接报价..." className="flex-1 bg-gray-100 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-brand-blue border border-transparent"/><button onClick={handlePostComment} className="bg-brand-blue text-white px-4 rounded-xl font-bold text-sm">发送</button></div></div>
        </div>
    );
    ...
    const renderJobsView = () => {
        const dates = [{ day: "周五", date: "02", label: "Today", active: true }, { day: "周六", date: "03", label: "Tomorrow", active: false }, { day: "周日", date: "04", label: "Sun", active: false }, { day: "周一", date: "05", label: "Mon", active: false }, { day: "周二", date: "06", label: "Tue", active: false }];
        return (
            <div className="flex flex-col h-full bg-gray-50 animate-slide-up">
                <div className="bg-gray-800 p-6 pt-12 pb-8 rounded-b-3xl shadow-lg z-10"><div className="flex justify-between items-end mb-4"><div><h2 className="text-2xl font-bold text-white mb-1">任务管理</h2><p className="text-gray-400 text-xs">今日安排 • 3 个任务</p></div><div className="text-right"><p className="text-gray-400 text-[10px] uppercase tracking-wider">预计收入</p><p className="text-xl font-bold text-green-400">RM 400.00</p></div></div><div className="flex justify-between gap-2 overflow-x-auto no-scrollbar pt-2">{dates.map((d, i) => (<div key={i} className={`flex flex-col items-center justify-center min-w-[60px] h-16 rounded-xl border transition cursor-pointer ${d.active ? 'bg-brand-orange border-brand-orange text-white shadow-lg scale-105' : 'bg-gray-700/50 border-gray-600 text-gray-400'}`}><span className="text-[10px] font-medium">{d.day}</span><span className="text-lg font-bold">{d.date}</span></div>))}</div></div>
                <div className="px-4 mt-4"><div className="bg-white p-1 rounded-xl shadow-sm flex">{['进行中/待办', '历史订单'].map((f) => (<button key={f} onClick={() => setJobFilter(f === '历史订单' ? 'history' : 'today')} className={`flex-1 py-2 text-xs font-bold rounded-lg transition ${(jobFilter === 'today' && f.includes('进行中')) || (jobFilter === 'history' && f.includes('历史')) ? 'bg-gray-800 text-white shadow' : 'text-gray-500 hover:bg-gray-50'}`}>{f}</button>))}</div></div>
                <div className="flex-1 overflow-y-auto p-4 space-y-4 pb-24">
                    {sifuSchedule.filter(job => jobFilter === 'history' ? job.status === 'completed' : job.status !== 'completed').map((job) => (<div key={job.id} className={`relative bg-white p-0 rounded-2xl shadow-sm border border-gray-100 overflow-hidden group transition active:scale-95 ${job.type === 'SOS' && job.status === 'active' ? 'ring-2 ring-red-500/20' : ''}`}><div className={`absolute left-0 top-0 bottom-0 w-1.5 ${job.status === 'completed' ? 'bg-green-500' : job.status === 'active' ? 'bg-blue-500' : job.type === 'SOS' ? 'bg-red-500' : 'bg-yellow-400'}`}></div><div className="p-4 pl-6 flex gap-4"><div className="flex flex-col items-center pt-1 min-w-[50px]"><span className="text-lg font-bold text-gray-800 leading-none">{job.time}</span><span className="text-[10px] text-gray-400 mt-1 uppercase">{job.date === 'Today' ? '今天' : '明天'}</span>{job.status === 'completed' && <div className="mt-2 text-green-500"><CheckCircle size={16}/></div>}</div><div className="flex-1 border-l border-gray-100 pl-4"><div className="flex justify-between items-start mb-1"><span className={`text-[10px] font-bold px-2 py-0.5 rounded ${job.type === 'SOS' ? 'bg-red-100 text-red-600' : 'bg-blue-50 text-brand-blue'}`}>{job.type === 'SOS' ? '⚡ 紧急救援' : '📅 预约服务'}</span><span className="font-bold text-gray-800">{job.price}</span></div><h3 className="font-bold text-gray-800 text-md mb-0.5">{job.category}</h3><div className="flex items-center gap-1 text-xs text-gray-500 mb-2"><User size={12}/> {job.customer}</div><div className="flex items-start gap-1 text-xs text-gray-400 leading-snug"><MapPin size={12} className="mt-0.5 flex-shrink-0"/> {job.address}</div></div></div>{job.status !== 'completed' && (<div className="bg-gray-50 p-2 flex border-t border-gray-100"><button className="flex-1 flex items-center justify-center gap-2 py-2 text-xs font-bold text-gray-600 hover:bg-white rounded-lg transition"><Phone size={14}/> 联系</button><div className="w-px bg-gray-200 my-1"></div><button className="flex-1 flex items-center justify-center gap-2 py-2 text-xs font-bold text-gray-600 hover:bg-white rounded-lg transition"><Navigation size={14}/> 导航</button>{job.status === 'active' ? (<button onClick={() => setActiveTab('home')} className="flex-1 ml-2 bg-brand-blue text-white rounded-lg text-xs font-bold shadow-md">继续工作</button>) : (<button className="flex-1 ml-2 bg-gray-800 text-white rounded-lg text-xs font-bold shadow-md">开始接单</button>)}</div>)}</div>))}
                    {sifuSchedule.filter(job => jobFilter === 'history' ? job.status === 'completed' : job.status !== 'completed').length === 0 && (<div className="flex flex-col items-center justify-center h-48 text-gray-400"><Clock size={40} className="mb-2 opacity-20"/><p className="text-sm">暂无相关任务</p></div>)}
                </div>
            </div>
        );
    };
	
	// 5. 个人中心视图 (Profile & Settings)
                ...
                <div className="p-6 pt-12 pb-8 bg-gray-800 rounded-b-3xl shadow-lg z-10">
                    <div className="flex items-start gap-4 mb-6">
                        <div className="relative">
                            ...
                            <div className="absolute -bottom-2 -right-2 bg-brand-blue text-white text-[10px] px-2 py-1 rounded-full border-2 border-gray-800 font-bold flex items-center gap-1">
                                <Shield size={10} fill="currentColor"/> 已认证
                            </div>
                        </div>
                        <div className="flex-1">
                            <h2 className="text-2xl font-bold">Alex Ong</h2>
                            <p className="text-gray-400 text-sm mb-2">ID: 882394 • 加入于 2021</p>
                            <div className="flex gap-2">
                                <span className="bg-yellow-500/20 text-yellow-500 px-2 py-0.5 rounded text-xs font-bold border border-yellow-500/30 flex items-center gap-1"><Award size={12}/> 金牌师傅</span>
                                <span className="bg-green-500/20 text-green-500 px-2 py-0.5 rounded text-xs font-bold border border-green-500/30">接单分 100</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-gray-700/50 p-4 rounded-xl border border-gray-600">
                        <div className="flex justify-between text-xs mb-2">
                            <span className="text-gray-300">距离 <span className="text-white font-bold">钻石师傅</span> 还差 12 单</span>
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-6 pb-24">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                        <div className="bg-gray-800 p-3 rounded-xl border border-gray-700 text-center">
                            ...
                            <div className="text-[10px] text-gray-500 mt-1">综合评分</div>
                        </div>
                        <div className="bg-gray-800 p-3 rounded-xl border border-gray-700 text-center">
                            <div className="text-white font-bold text-xl">100%</div>
                            <div className="text-[10px] text-gray-500 mt-1">准时率</div>
                        </div>
                        <div className="bg-gray-800 p-3 rounded-xl border border-gray-700 text-center">
                            <div className="text-white font-bold text-xl">328</div>
                            <div className="text-[10px] text-gray-500 mt-1">总服务单</div>
                        </div>
                    </div>

                    {/* Settings Lists */}
                    <div className="space-y-3">
                        <h3 className="text-xs font-bold text-gray-500 uppercase tracking-wider ml-1">服务管理</h3>
                        <div className="bg-gray-800 rounded-2xl overflow-hidden border border-gray-700">
                            {[
                                { l: "服务技能与报价", i: <Wrench size={18} className="text-blue-400"/>, d: "修改定价 / 添加技能" },
                                { l: "服务范围设置", i: <MapPin size={18} className="text-green-400"/>, d: "当前: 10km" },
                                { l: "工作时间", i: <Clock size={18} className="text-orange-400"/>, d: "每天 9:00 - 18:00" },
                            ].map((item, idx) => (
                                <div key={idx} className="flex items-center p-4 border-b border-gray-700 last:border-0 hover:bg-gray-750 active:bg-gray-700 transition cursor-pointer">
                        ...
                        <h3 className="text-xs font-bold text-gray-500 uppercase tracking-wider ml-1 mt-6">账号与安全</h3>
                        <div className="bg-gray-800 rounded-2xl overflow-hidden border border-gray-700">
                             {[
                                { l: "收款银行卡", i: <CreditCard size={18} className="text-purple-400"/>, d: "Maybank **** 8823" },
                                { l: "实名认证信息", i: <FileText size={18} className="text-gray-400"/>, d: "已通过" },
                                { l: "App 设置", i: <Settings size={18} className="text-gray-400"/>, d: "语言 / 通知" },
                            ].map((item, idx) => (
                                <div key={idx} className="flex items-center p-4 border-b border-gray-700 last:border-0 hover:bg-gray-750 active:bg-gray-700 transition cursor-pointer">
                                    <div className="w-10 h-10 rounded-full bg-gray-700 flex items-center justify-center mr-3">{item.i}</div>
                                    <div className="flex-1">
    ...
    return (
        <div className="flex flex-col h-full bg-gray-900 text-white animate-fade-in relative md:flex-row w-full">
            ...
            <div className="flex-1 flex flex-col h-full overflow-hidden relative">
                ...
                {activeTab === 'jobs' && renderJobsView()} {/* ✅ 修复了这里！ */}
                                    ...
                                    {requestList.map(req => (
                                        <div key={req.id} onClick={() => setSelectedRequest(req)} className="bg-gray-800 p-4 rounded-xl border border-gray-700 cursor-pointer hover:border-gray-600 transition">
                                            <div className="flex justify-between items-start mb-3">
                                                <div className="flex items-center gap-3"><div className="w-10 h-10 bg-gray-700 rounded-full flex items-center justify-center text-sm font-bold text-gray-300">{req.user[0]}</div><div><h4 className="font-bold text-white text-sm">{req.user}</h4><p className="text-[10px] text-gray-400">{req.time} • {req.dist}</p></div></div>
                                            ...
                                            <div className="flex items-center justify-between border-t border-gray-700 pt-3"><span className="text-xs text-gray-500 flex items-center gap-1"><MapPin size={12}/> {req.address}</span><div className="flex items-center gap-1 text-xs text-brand-orange font-bold"><MessageSquare size={14}/> {req.comments.length} 留言报价</div></div>
                                        </div>
                                    ))}
                                </div>
                            </div>
			  ...
			  <div className="fixed inset-0 z-[120] bg-black/50 flex items-end md:items-center md:justify-center animate-fade-in">
				<div className="bg-white w-full rounded-t-3xl p-6 pb-[110px] animate-slide-up md:rounded-2xl md:w-[420px] md:pb-6 md:shadow-2xl">
				  <div className="flex justify-between items-center mb-4">
					<h3 className="font-bold text-lg text-gray-800">发布需求帖</h3>
				  ...
				  <div className="space-y-3">
					<div>
					  ...
					  >
						<option value="房屋修缮">房屋修缮</option>
						<option value="冷气服务">冷气服务</option>
						<option value="水管维修">水管维修</option>
						<option value="电工服务">电工服务</option>
						<option value="开锁服务">开锁服务</option>
					  </select>
					</div>

					<div>
					  <label className="text-xs font-bold text-gray-400 uppercase">标题</label>
					  <input
						value={newRequest.title}
						onChange={(e) => setNewRequest(prev => ({ ...prev, title: e.target.value }))}
						placeholder="例如：厨房水管漏水，需要尽快处理"
						className="w-full mt-1 bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-brand-blue"
					  />
					</div>
